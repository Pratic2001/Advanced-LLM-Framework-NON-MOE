#!/usr/bin/env python3
"""
train_pretrain_deepspeed.py

DeepSpeed-powered pretraining for the generic dense LLM from model.py.
...
configuration for the hardware it finds:
  ...
  ZeRO-3  -- shard optimizer states + gradients + model parameters
             required for very large models (params > 2x VRAM per GPU)
...
Launch:
    # Single node, 1 GPU
    deepspeed train_pretrain_deepspeed.py --model-size 0.6B --data-dir ./packed

    # Single node, 4 GPUs
    deepspeed --num_gpus 4 train_pretrain_deepspeed.py --model-size 1.7B --data-dir ./packed
    ...
    deepspeed train_pretrain_deepspeed.py --model-size 4B --zero-stage 3 \
        --cpu-offload-optimizer --data-dir ./packed

    # Resume
    deepspeed train_pretrain_deepspeed.py --resume ./checkpoints/latest_ds/ --data-dir ./packed
"""
...
def _tflops_from_cuda_props(props) -> Tuple[float, str]:
    ...
    Formula:
        peak_bf16_tflops = 2 x SM_count x (CUDA_cores_per_SM x 2) x boost_clock
        ^ x2 for FMA,  ^ x2 for bf16 tensor-core 2:1 throughput vs FP32

    For Tensor Core architectures (CC >= 7.0) the tensor core multiplier is
    applied based on the generation:
        Volta  (7.0): 8x FP16 tensor core vs FP32 CUDA core throughput
        Turing (7.5): 8x
        Ampere (8.x): 2x bf16 vs FP32 (bf16 is supported natively)
        Ada    (8.9): same as Ampere for bf16
        Hopper (9.0): 2x bf16 (+ FP8 paths, not relevant here)
    ...
    if cc_major >= 9:        # Hopper -- bf16 TC is ~2x FP32 scalar
        bf16_mult = 2.0
    elif cc_major == 8:      # Ampere / Ada -- bf16 TC ~2x FP32 scalar
        bf16_mult = 2.0
...
def audit_hardware() -> dict:
    ...
    for i in range(n_gpus):
        ...
        info["gpus"].append({
            ...
            "tflops_source": tflops_source,   # tells user how accurate MFU is
            "bw_gb_s":       bw_gb_s,
...
def select_zero_stage_and_offload(
    ...
    the model size:
        ...
        full_fit   = params x (2 + 2 + 8) / n_gpus      # weights + grads + Adam
        zero2_fit  = params x (2 + 2)     / n_gpus      # weights + grads (no Adam per GPU)
        zero3_fit  = params x 2           / n_gpus      # weights only (sharded)
...
def build_ds_config(
    ...
    # `build_scheduler` and the "LR override" section in train()). A DS
    ...
    # (N micro-batches of forward+backward, then a single engine.step()).
    # If we left gradient_accumulation_steps=N in the config, DeepSpeed
    # would ALSO try to count micro-batches internally -- but since
    # engine.step() is only called once per N microbatches, DS's
    ...
    if getattr(args, "tensorboard", False):
        cfg["tensorboard"] = {
            "enabled": True,
            "output_path": os.path.join(getattr(args, "out_dir", "./checkpoints_ds"),
                                        "tensorboard"),
            "job_name": "pretrain_deepspeed",
        }
...
class PackedDataLoader:
    """
    Memmap-backed, prefetched, rank-sharded, per-loader independent RNG,
    and a sequential iterator for true held-out evaluation. Optionally uses a
    multi-process DataLoader pool (num_workers>0) for higher throughput on
...
def get_lr(step: int, scheduler) -> float:
    """Return the learning rate for the given step using build_scheduler()."""
...
def save_ds_checkpoint(
    ...
    if recipe is not None:
        recipe_path = os.path.join(path, "recipe.json")
...
def load_ds_checkpoint(engine, resume_path: str) -> Tuple[int, float, dict]:
    """
    Load a DeepSpeed checkpoint and return (step, best_val_loss, meta_dict).
    """
    ...
    return step, best_val_loss, meta
...
def evaluate(engine, val_loader, eval_steps, device, world_size,
             use_cudagraphs: bool = False) -> tuple:
    """
    Returns (loss_random, loss_sequential). Short version: loss_random is
    comparable to train loss (both are random-window), loss_sequential is
    the true held-out loss a downstream consumer would see.
...
def validate(model, val_loader, pad_id=0):
    """
    Standalone sequential evaluation for non-DeepSpeed contexts.
    Returns the mean cross-entropy loss over the validation set.
    """
    device = next(model.parameters()).device
    model.eval()
    losses = []
    with torch.no_grad():
        for x, y in val_loader.iter_sequential(device, max_batches=100):
            out = model(x)
...
def smoke_test():
    """
    Run a minimal training loop with synthetic data to verify the full
    pipeline (model, optimizer, DeepSpeed config, checkpoint save/load)
    works end-to-end.
    """
    ...
    print(f"Smoke-test model: {n_params:,} params on {device}")

    # Manual optimizer (not DeepSpeed) for the smoke test
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=3e-4, weight_decay=0.1, betas=(0.9, 0.95), eps=1e-8,
    )
    scheduler = build_scheduler("cosine", warmup_steps=2, max_steps=5,
                                peak_lr=3e-4, min_lr=3e-5)
    ctx = (torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16)
           if device.type == "cuda" else nullcontext())

    loader     = PackedDataLoader(os.path.join(packed, "train.bin"), 64, 2,
                                  dtype=np.uint16, loader_id=0)
    val_loader = PackedDataLoader(os.path.join(packed, "val.bin"),   64, 2,
                                  dtype=np.uint16, loader_id=1)
    loader.prime()
    val_loader.prime()

    gpu_peak = GPU_PEAK_TFLOPS.get(
        torch.cuda.get_device_name(0) if torch.cuda.is_available() else "", 0.0
    )
    ...
    }, os.path.join(ckpt_dir, "smoke_test.pt"))

    model2 = TransformerForCausalLM(config).to(device)
    ckpt = torch.load(os.path.join(ckpt_dir, "smoke_test.pt"), map_location=device,
                       weights_only=True)
...
def train(args):
    ...
    if args.resume:
        ...
        config = ModelConfig(**ckpt_meta["config"])
    ...
    # gradient magnitudes shrink, the bf16-accumulated sum over N
    ...
    # ---------------------------------------------------------------- LR scheduler
    # Use our own scheduler from optim.lr_schedule -- driving it manually
    # via param_groups override rather than letting DeepSpeed's built-in
    # scheduler manage LR (see the long comment in build_ds_config).
    ...
    if getattr(args, "recipe", None):
        try:
            ...
            if master:
                print(f"[Recipe] loaded from {args.recipe}: mode={recipe.mode}")
        except Exception as e:
            if master:
                print(f"[Recipe] failed to load: {e}")
    ...
    total_tokens  = 0
    if args.resume:
        start_step, best_val_loss, resume_meta = load_ds_checkpoint(engine, args.resume)
        total_tokens = resume_meta.get("total_tokens", 0)
    ...
    if master and args.wandb_project:
        try:
            ...
            wandb.init(
                ...
                name=args.wandb_run_name or f"densellm-{args.model_size}-z{zero_stage}",
    ...
    for step in range(start_step, args.max_steps):
        ...
        for micro_step in range(args.grad_accum_steps):
            ...
            # Track total tokens (all ranks contribute equally)
        ...
        if step % args.eval_interval == 0 and step > start_step:
            ...
            val_loss = val_seq
            if master:
                improved = " [improved]" if val_loss < best_val_loss else ""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="DeepSpeed pretraining for generic dense LLM."
    )
    ...
    p.add_argument("--recipe", default=None,
                   help="Path to a recipe.json file for training configuration.")
