...
export default function DeepSpeedJobsPage() {
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold">DeepSpeed Jobs</h1>
        <p className="text-muted-foreground text-sm mt-1">
          Monitor and manage DeepSpeed training jobs
        </p>
      </div>
      ...
      {jobs.length === 0 ? (
        ...
        >
          ...
          <p className="text-sm text-muted-foreground max-w-md mx-auto">
            Launch a DeepSpeed training job to see its progress, logs, and metrics here.
          </p>
          <Link
            ...
            <motion.div key={job.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass rounded-xl p-5 border border-border/50">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">{job.type} • ZeRO-{job.zeroStage}</p>
                </div>
