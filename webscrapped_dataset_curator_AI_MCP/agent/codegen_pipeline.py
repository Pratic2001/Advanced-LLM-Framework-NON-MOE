...
def call_ollama(prompt: str, timeout: float = 240.0, log_path: Optional[str] = None,
    ...
    payload = {
        ...
        "prompt": prompt,
        "stream": True,
        # Reasoning-capable models (qwen3, deepseek-r1, ornith, ...) emit a
        # <think>...</think> block by default. This call expects the
        # response to be nothing but a python code block; if thinking prose
        # leaks in (or a partial/mismatched fence swallows some of it),
        # _extract_code_block() can end up returning a mix of reasoning
        # text and real code, which is exactly what produces syntax errors
        # like "return outside function" -- the "return" was actually
        # inside a function in the model's real code, but got separated
        # from its enclosing def by leaked reasoning lines. Disable
        # thinking explicitly so `response` is only ever the code.
        # Harmless no-op on models without extended thinking.
        "think": False,
    }
    chunks = []
    try:
...
def _extract_code_block(text: str) -> str:
    ...
    Belt-and-suspenders against reasoning-model leakage: `think: false` in
    call_ollama's payload is supposed to suppress <think> blocks, but on
    some Ollama versions/models it doesn't fully suppress it. Strip any
    <think>...</think> span (including an unterminated trailing one)
    before fence-matching, so a leaked reasoning line never ends up
    spliced into the returned code."""
