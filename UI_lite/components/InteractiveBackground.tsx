...
// ── Palette-aware interactive background (lite — canvas 2D only) ────────────
//
// UI_lite strips the per-palette WebGL worlds and renders every palette's
// background through the 2D canvas strategies below. Each palette id maps to
// a distinct "feel" rendered into a single canvas. Strategies are
// self-contained: they own their own particle arrays, advance their own
// state, and render to the provided 2D context.
...
// UI_lite: every palette renders through the 2D canvas strategies. deep-space
// is lite's richest-looking strategy (embers reads the palette colours), since
// the WebGL cosmos that normally owns it is not present here.
...
export function InteractiveBackground({
  ...
  // Mouse + touch tracking (drives the 2D canvas interactions)
  useEffect(() => {
