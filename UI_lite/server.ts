...
app.prepare().then(() => {
  const server = createServer((req, res) => {
    ...
    // Tailscale Funnel mounts this app under BASE_PATH (e.g. /lite) and
    ...
    if (bp && parsedUrl.pathname && !parsedUrl.pathname.startsWith(bp)) {
      // A funnel-stripped root ("/") would re-prepend to "/lite/", which Next
      // 308s to "/lite" — and since that is the very URL the funnel already
